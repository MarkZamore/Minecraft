...
Push в `main` запускает GitHub Actions. Workflow собирает приложение, создаёт `update.json` и публикует только `Minecraft.exe` и manifest обновления в release `latest`.

При каждом запуске приложение проверяет уже скачанное обновление, затем публичный GitHub Release. Новая версия автоматически скачивается в фоне и проверяется по размеру и SHA-256. Установка выполняется только после нажатия `Обновить`: приложение заменяет `Minecraft.exe` и перезапускается. Проверенный файл сохраняется между запусками, поэтому его можно установить без интернета. Если GitHub недоступен, приложение не показывает ошибку и продолжает работу в состоянии `Вы на последней версии`.

...
public sealed class VoiceNetworkCoordinator
{
    ...
    public bool ShouldProtectVoice
    {
        get
        {
            lock (_gate) return _joined && _connectedPeers > 0;
        }
    }
    ...
    public VoiceNetworkSnapshot Snapshot
    {
        get
        {
            lock (_gate)
            {
                return new VoiceNetworkSnapshot(_joined, _connectedPeers, _lossPercent, _jitterMs, _rttMs);
            }
        }
    }
...
public sealed class VoiceTransferLimiter
{
    ...
    public async ValueTask ThrottleAsync(int bytes, CancellationToken token)
    {
        ...
        if (!snapshot.IsJoined || snapshot.ConnectedPeers == 0)
        {
